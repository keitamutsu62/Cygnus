...
function CancelModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        style={{ width: '100%', maxWidth: 480, background: 'var(--bg)', borderTop: `1px solid ${goldBorder}`, borderRadius: '16px 16px 0 0', padding: '20px 20px 48px' }}
      ...
      >
        ...
        <button
          onClick={onClose}
          style={{ width: '100%', padding: 14, background: gold, border: 'none', borderRadius: 2, fontFamily: zen, fontSize: 15, color: 'var(--on-accent)', cursor: 'pointer' }}
        >
          キャンセル（解約しない）
        </button>
      </div>
